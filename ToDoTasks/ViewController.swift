...
class ViewController: UIViewController{
    ...
    Todo(title: "Finish HW for CS300!"),
    Todo(title: "Call mom!")
    ]
...
extension ViewController: UITableViewDelegate{
    ...
    cell.set(title: todo.title, isOn: todo.isComplete)
    return cell
}
   ...
   extension ViewController: CheckTableViewCellDelegate{
    ...
    func checkTableViewCell(_ cell: CheckTableViewCell, didChangeCheckedState isOn: Bool) {
        ...
        let todo = todos[indexPath.row]
        let newTodo = Todo(title: todo.title, isComplete: isOn)
